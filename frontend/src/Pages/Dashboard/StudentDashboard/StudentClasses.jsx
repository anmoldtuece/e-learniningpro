...
// import { TbCalendarEvent, TbClock, TbUser } from 'react-icons/tb';
...
//                         <p className='text-gray-600 mt-1'>Your weekly timetable and class links</p>
//                     </div>
//                 </div>
...
//                                                             </a>
//                                                         ) : (
//                                                             <span className="text-red-500 font-semibold">No link provided</span>
//                                                         )}
//                                                     </div>
//                                                 </div>
//                                             </div>
...
//                                 <p className='text-gray-500'>Check back later for your schedule</p>
//                             </div>
//                         )}
//                     </div>
//                 </div>
...
//                 {/* Next Class Card */}
//                 {weeklyClasses.length > 0 && (
//                     <NavLink to={weeklyClasses[0]?.link || '#'} target='_blank'>
//                         <div className='bg-white p-5 mt-8 cursor-pointer rounded-lg text-black shadow-lg'>
...
//                                     {typeof weeklyClasses[0]?.date === 'string' ? weeklyClasses[0]?.date.slice(0, 10) : ''}
//                                 </span>
//                                 <span className='text-[#018280] text-2xl ml-2'>
//                                     {typeof weeklyClasses[0]?.timing === 'number' ? formatTime(weeklyClasses[0]?.timing) : ''}
//                                 </span>
//                             </div>
//                             <div className='flex gap-12 items-center'>
//                                 <div className='ml-3'>
//                                     <p>Your next Class</p>
//                                     <p className='text-[#018280] text-3xl font-semibold'>{weeklyClasses[0]?.coursename}</p>
//                                     <p className='text-light-blue-700'>{weeklyClasses[0]?.title.slice(0, 25)} ...</p>
//                                 </div>
...
function StudentClasses() {
    ...
    // Filter for all upcoming classes from today
    const upcomingClasses = Array.isArray(data)
        ? data.filter(clas => {
            if (!clas || !clas.date) return false;
            const classDate = new Date(clas.date.slice(0, 10));
            const today = new Date();
        ...
        <div className='min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-6'>
            <div className='max-w-4xl mx-auto space-y-8'>
                ...
                <div className='flex items-center gap-3 mb-6'>
                    ...
                    <div>
                        <h1 className='text-3xl font-bold text-gray-800'>My Classes</h1>
                        <p className='text-gray-600 mt-1'>Your upcoming timetable and class links</p>
                    </div>
                </div>
                ...
                <div className='bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden'>
                    <div className='bg-gradient-to-r from-blue-600 to-indigo-700 px-6 py-4'>
                        ...
                        <p className='text-blue-100 mt-1'>Your classes from today onwards</p>
                    </div>
                    <div className='p-6'>
                ...
                {/* Next Class Card */}
                {upcomingClasses.length > 0 && (
                    <NavLink to={upcomingClasses[0]?.link || '#'} target='_blank'>
                        <div className='bg-white p-5 mt-8 cursor-pointer rounded-lg text-black shadow-lg'>
                            <div className='flex gap-3 items-center mb-5 mt-2'>
                                <img src={Clock} alt="clock" width={50} />
                                <span className='text-[#4E84C1] text-2xl font-semibold'>
                                    {upcomingClasses[0]?.date.slice(0, 10)}
                                </span>
                                <span className='text-[#018280] text-2xl ml-2'>
                                    {formatTime(upcomingClasses[0]?.timing)}
                                </span>
                            </div>
                            <div className='flex gap-12 items-center'>
                                <div className='ml-3'>
                                    <p>Your next Class</p>
                                    <p className='text-[#018280] text-3xl font-semibold'>{upcomingClasses[0]?.coursename}</p>
                                    <p className='text-light-blue-700'>{upcomingClasses[0]?.title.slice(0, 25)} ...</p>
                                </div>
                                <img src={Camera} alt="Camera" width={70} />
                            </div>
                        </div>
