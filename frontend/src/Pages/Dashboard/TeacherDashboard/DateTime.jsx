...
const DateTime = ({ setDate, allowedDays }) => {
  ...
  // Check if the date's day is in allowedDays array
  const isAllowedDate = (date) => {
    return allowedDays.some((allowedDay) => allowedDay.day === date.getDay());
  };
  ...
  const filterTime = (time) => isAllowedTime(time);

  useEffect(() => {
    if (selectedDate) {
      // Format datetime in local time (no 'Z')
      const formattedDate = format(selectedDate, "yyyy-MM-dd'T'HH:mm:ss");
      setDate(formattedDate); // local datetime
    }
