...
//             <br>
//             <a href="${process.env.VITE_BACKEND_URL}/api/teacher/verify?id=${createdTeacherId}">
...
const verifyEmail = async (Email, Firstname, createdTeacherId) => {
  try {
    if (!Email || !Firstname || !createdTeacherId) {
      throw new ApiError(400, "Missing required parameters");
    }

    const verificationLink = `${process.env.VITE_BACKEND_URL}/api/teacher/verify?id=${createdTeacherId}`;

    const htmlMessage = `
      <div style="text-align: center;">
        <p style="margin: 20px;"> Hi ${Firstname}, Please click the button below to verify your E-mail. </p>
        <img src="https://img.freepik.com/free-vector/illustration-e-mail-protection-concept-e-mail-envelope-with-file-document-attach-file-system-security-approved_1150-41788.jpg?size=626&ext=jpg&uid=R140292450&ga=GA1.1.553867909.1706200225&semt=ais" 
    ...
    const result = await Sendmail(Email, "Verify your E-mail", htmlMessage);
